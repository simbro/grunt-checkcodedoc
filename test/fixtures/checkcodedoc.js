...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('checkcodedoc', 'Checks a codebase to make sure all methods are documented properly', function() {
            ...
            for (var i = 0; i < fileLength; i++) {
                ...
                if (captureLines) {
                    ...
                    if (lines[i].match(/^\s+\*\/\s*$/)) {
                        ...
                        if (!methodSignature) {
                            // methodSignature = lines[i + 1].match(/\s+(.+)\s*[=:]\sfunction\s*\((.*)\)/);
         ...
         * @param {String} The relative path to the log file to write to
         */
        var textReport = function(errorReport, reporterOutput) {
            ...
            grunt.file.write(reporterOutput, report);
        };
         ...
         *
         * @private
         * @param {Integer} The collection of errors as object literals
         * @param {String} The relative path to the log file to write to
         */
        var jsonReport = function(errorReport, reporterOutput) {
         ...
         * @param {Array} The collection of errors as object literals
         */
        var xmlReport = function(errorReport, reporterOutput) {
            var options = {
                arrayMap: {
                    files: "file",
                    errors: "error"
            ...
            var output = js2xmlparser("files", errorReport, options);
        ...
        switch(options.reporter) {
            case 'text':
                textReport(errorReport, options.reporterOutput);
                break;
            case 'json':
                jsonReport(errorReport, options.reporterOutput);
                break;
            case 'xml':
                xmlReport(errorReport, options.reporterOutput);
                break;
            default:
